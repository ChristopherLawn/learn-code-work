async function addLanFormHandler(event) {
  ...
  const icon_data = document.querySelector('#icon-name').value.trim();
  console.log("i was herreeeeeee");
  if (name && description && major_organizations && developed_by && year_created && version_number && filename_extension && licensed_under && website && language_type && icon_data) {
